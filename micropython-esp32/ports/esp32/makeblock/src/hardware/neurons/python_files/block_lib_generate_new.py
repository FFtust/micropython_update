...
class file_head_ope(object):
    ...
    def create_blocks_index(self):
        ...
        for item in NEURONS_SPECIAL_LIB:
            ...
            self.blocks_index_offset += (item[0]["respond_num"] + item[0]["command_num"]) * 6 + 2  
    ...
    def file_write_head(self):
        ...
        # write magic data 16 bytes
        lib_magic_l = block_lib_info_list_fill(list(HEAD_MAGIC_RECORD[-1]), HEAD_MAGIC_STR_LENGTH, 0x00) 
        for da in lib_magic_l:
            if type(da) == str:
...
class blocks_lib(file_head_ope):
    ...
    def write_single_special_block_info(self, f, block_info, offset):
        f.seek(offset)
        print("write index is ", offset )
        f.write(pack('B', block_info["type"]))
        f.write(pack('B', block_info["sub_type"]))
    ...
    def write_special_block_info(self):
        ...
        for item in NEURONS_SPECIAL_LIB:
            ...
            self.write_single_special_block_info(f, block, off)
        f.close()        
